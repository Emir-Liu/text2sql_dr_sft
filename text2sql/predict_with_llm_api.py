...
from text2sql.llm_api_operator import LLMOperator
...
def inference(model_operator, dataset_loader):
    ...
    for item in tqdm(dataset_loader):
        try:
            ...
            total_prompt = PART_PROMPT.format(
                INSTRUCTION_PROMPT.format(instructs), inputs
            )
            # response = model_operator.generate(total_prompt)
            response = model_operator.chat_with_prompt(content=total_prompt)
            print(f"response:{response}")
...
def predict_dataset(
    ...
    print(f"predict path:{predict_path}")
    # load dataset
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", help="dataset name", default=False)
    parser.add_argument(
        "--base_model_name_or_path", help="base model name or path", default=None
    )
    parser.add_argument("--adapter_name", help="base model name or path", default=None)
    parser.add_argument(
    ...
    predict_dataset(
        predict_file_name=pred_file_name,
